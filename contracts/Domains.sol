...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Geek Name Service", "GNS") {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Geek name service", "image": "data:image/svg+xml;base64,',
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0, "Domain name must be at least one character long");
        if (len == 3) {
